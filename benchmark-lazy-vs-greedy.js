...
console.log(`Testing range: ${START_NUMBER} to ${END_NUMBER} (${SAMPLE_SIZE.toLocaleString()} numbers)`);
...
function showProgress() {
  ...
  process.stdout.write(`\r[${progressBar}] ${percentage}% (${processed.toLocaleString()}/${SAMPLE_SIZE.toLocaleString()})`);
}
...
for (let goal = START_NUMBER; goal <= END_NUMBER; goal++) {
  ...
  if (lazyResult.success) {
    if (lazyResult.solvable) {
      ...
      results.lazy.algorithms[lazyResult.algorithm] = (results.lazy.algorithms[lazyResult.algorithm] || 0) + 1;
    }
  } else {
    results.lazy.errors++;
  }
  ...
  if (greedyResult.success) {
    if (greedyResult.solvable) {
      ...
      results.greedy.algorithms[greedyResult.algorithm] = (results.greedy.algorithms[greedyResult.algorithm] || 0) + 1;
    }
  } else {
  ...
  if (lazyResult.success && greedyResult.success) {
    if (lazyResult.solvable && greedyResult.solvable) {
      if (lazyResult.moves === greedyResult.moves && lazyResult.algorithm === greedyResult.algorithm) {
...
results.lazy.averageMoves = results.lazy.solvableCount > 0 ? results.lazy.totalMoves / results.lazy.solvableCount : 0;
results.greedy.averageMoves = results.greedy.solvableCount > 0 ? results.greedy.totalMoves / results.greedy.solvableCount : 0;
...
console.log(`Speed Improvement:  ${((results.greedy.totalTime / results.lazy.totalTime) * 100).toFixed(1)}% (lazy vs greedy)`);
console.log(`Lazy Avg/Number:    ${(results.lazy.totalTime / SAMPLE_SIZE).toFixed(3)} ms`);
console.log(`Greedy Avg/Number:  ${(results.greedy.totalTime / SAMPLE_SIZE).toFixed(3)} ms`);

// Solvability comparison
console.log('\n✅ SOLVABILITY COMPARISON');
console.log('-'.repeat(40));
console.log(`Lazy Solvable:      ${results.lazy.solvableCount.toLocaleString()} / ${SAMPLE_SIZE.toLocaleString()} (${((results.lazy.solvableCount / SAMPLE_SIZE) * 100).toFixed(1)}%)`);
console.log(`Greedy Solvable:    ${results.greedy.solvableCount.toLocaleString()} / ${SAMPLE_SIZE.toLocaleString()} (${((results.greedy.solvableCount / SAMPLE_SIZE) * 100).toFixed(1)}%)`);
...
console.log('\n🎯 SOLUTION QUALITY COMPARISON');
console.log('-'.repeat(40));
console.log(`Same Solutions:     ${results.comparison.sameSolution.toLocaleString()} (${((results.comparison.sameSolution / SAMPLE_SIZE) * 100).toFixed(1)}%)`);
console.log(`Different Moves:    ${results.comparison.differentMoves.toLocaleString()} (${((results.comparison.differentMoves / SAMPLE_SIZE) * 100).toFixed(1)}%)`);
console.log(`Lazy Better:        ${results.comparison.lazyBetter.toLocaleString()} cases`);
console.log(`Greedy Better:      ${results.comparison.greedyBetter.toLocaleString()} cases`);
console.log(`Only Lazy Solved:   ${results.comparison.onlyLazySolved.toLocaleString()} cases`);
console.log(`Only Greedy Solved: ${results.comparison.onlyGreedySolved.toLocaleString()} cases`);
...
console.log(`Move Difference:    ${(results.greedy.averageMoves - results.lazy.averageMoves).toFixed(2)} (greedy - lazy)`);
...
Object.entries(results.greedy.algorithms)
  .sort((a, b) => b[1] - a[1])
  .forEach(([algorithm, count]) => {
    const percentage = ((count / results.greedy.solvableCount) * 100).toFixed(1);
    console.log(`  ${algorithm}: ${count.toLocaleString()} (${percentage}%)`);
  });
...
if (Math.abs(qualityDiff) < 0.1) {
  ...
  console.log(`📈 GREEDY finds better solutions (+${qualityDiff.toFixed(2)} avg moves)`);
} else {
  console.log(`📉 LAZY finds better solutions (${Math.abs(qualityDiff).toFixed(2)} fewer avg moves)`);
}
...
if (speedup > 1.5 && Math.abs(qualityDiff) < 0.5) {
  console.log('🏆 Use LAZY for exercise generation (faster with similar quality)');
...
console.log(`Total runtime: ${((results.lazy.totalTime + results.greedy.totalTime) / 1000).toFixed(1)} seconds`);